...
async fn main() {
        ...
        thread::spawn(move || loop {
            let mut slot = loop {
                match tx.try_send_ref() {
                    ...
                    Err(TrySendError::Closed(_)) => panic!("channel closed / other error"),
    ...
    // This task receives from the channel. After 20s, we simulate a slowdown by sleeping for 10ms
    // between receives.
    let async_recv_task = task::spawn(async move {
        let start = Instant::now();
        let mut slowed_down = false;

        while let Some(data) = rx.recv_ref().await {
            if slowed_down {
    ...
    // This task starts sending items in a loop after 30s.
